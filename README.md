```
                        __                           __               
                       / /________  _______  _______/ /____  ____ ___ 
                      / / ___/ __ \/ ___/ / / / ___/ __/ _ \/ __ `__ \
                     / (__  ) /_/ (__  ) /_/ (__  ) /_/  __/ / / / / /
                    /_/____/ .___/____/\__, /____/\__/\___/_/ /_/ /_/ 
                          /_/         /____/                          

                       ⚡ Designed for convenience and efficiency ⚡
```

Neovim lsp enhance plugin. Fork from [lspsaga](https://github.com/nvimdev/lspsaga.nvim)

[![](https://img.shields.io/badge/Element-0DBD8B?style=for-the-badge&logo=element&logoColor=white)](https://matrix.to/#/#lspsystem-nvim:matrix.org)

1. [Install](#install)
2. [Example Configuration](#example-configuration)
3. [Using Lspsystem](#using-lspsystem)
4. [Customizing Lspsystem's Appearance](#customizing-lspsystems-appearance)
5. [Backers](#backers)s
6. [License](#license)

## Install

You can use plugin managers like `lazy.nvim` and `packer.nvim` to install `lspsystem` and lazy load `lspsystem` using the plugin manager's keyword for lazy loading (`lazy` for `lazy.nvim` and `opt` for `packer.nvim`).

- `cmd` - Load `lspsystem` only when a `lspsystem` command is called.
- `ft` - `lazy.nvim` and `packer.nvim` both provide lazy loading by filetype. This way, you can load `lspsystem` according to the filetypes that you use a LSP in.
- `event` - Only load `lspsystem` on an event like `BufRead` or `BufReadPost`. Do make sure that your LSP plugins, like [lsp-zero](https://github.com/VonHeikemen/lsp-zero.nvim) or [lsp-config](https://github.com/neovim/nvim-lspconfig), are loaded before loading `lspsystem`.
- `dependencies` - For `lazy.nvim` you can set `nvim-jo/lspsystem.nvim` as a dependency of `nvim-lspconfig` using the `dependencies` keyword and vice versa. For `packer.nvim` you should use `requires` as the keyword instead.
- `after` - For `packer.nvim` you can use `after` keyword to ensure `lspsystem` only loads after your LSP plugins have loaded. This is not necessary for `lazy.nvim`.
...
\*\*If you are using Neovim 0.9 and want to disable the title, see [Customizing Lspsystem's Appearance](#customizing-lspsystems-appearance)
...
## :Lspsystem peek_definition

There are two commands, `:Lspsystem peek_definition` and `:Lspsystem goto_definition`. The `peek_definition`
...
Lspsystem can use it to render the hover window.
You can press the keyboard shortcut for `:Lspsystem hover_doc` twice to enter the hover window.
...
## :Lspsystem diagnostic_jump_next

Jumps to next diagnostic position and show a beacon highlight. Lspsystem will then show the code actions.
...
You can also use a filter when using diagnostic jump by using a Lspsystem function. The function takes a table as its argument.
...
## :Lspsystem show_diagnostics

`show_line_diagnostics`, `show_buf_diagnostics`, `show_workspace_diagnostics`
`show_cursor_diagnsotics`. and support an
argument `++unfocus` to make it unfocus. like `:Lspsystem show_workspace_diagnostics ++unfocus`
...
## :Lspsystem incoming_calls / outgoing_calls
...
## :Lspsystem symbols in a custom winbar/statusline
...
## Customizing Lspsystem's Appearance
...
All kinds used in Lspsystem are defined in [lspkind.lua](./lua/lspsystem/lspkind.lua).
...
Thanks for everyone!
[@Jo Project](https://github.com/jo-project)
