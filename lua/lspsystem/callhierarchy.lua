...
function ch:call_hierarchy(item, parent)
  ...
  client.request(self.method, { item = item }, function(_, res)
    ...
    local kind = require('lspsystem.lspkind').get_kind()
    if not parent then
      local icons = {}
      for i, v in pairs(res) do
        ...
        self.data[#self.data + 1] = {
          ...
          highlights = {
            ['SystemCollapse'] = { 0, #expand_collapse },
            ['SystemWinbar' .. kind[target.kind][1]] = { #expand_collapse, #expand_collapse + #icon },
          },
          winline = i + 1,
          expand = false,
          children = {},
...
function ch:expand_collapse()
  ...
  if not node.expand then
    if not node.requested and not self.pending_request then
      ...
      node.highlights['SystemCollapse'] = { unpack(node.highlights['SystemExpand']) }
      node.highlights['SystemExpand'] = nil
      vim.bo.modifiable = true
  ...
  node.expand = false
  vim.bo[self.bufnr].modifiable = false
  node.highlights['SystemExpand'] = { unpack(node.highlights['SystemCollapse']) }
  node.highlights['SystemCollapse'] = nil
...
function ch:render_win()
  ...
  local content_opt = {
    contents = content,
    filetype = 'lspsystemcallhierarchy',
    buftype = 'nofile',
    enter = true,
    border_side = {
      ['right'] = ' ',
