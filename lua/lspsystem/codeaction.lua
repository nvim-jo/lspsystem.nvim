...
function act:code_action(options)
  if self.pending_request then
    vim.notify(
      '[lspsystem.nvim] there is already a code action request please wait',
      vim.log.levels.WARN
    )
    return
  end
...
function act:do_code_action(num, tuple, enriched_ctx)
  ...
  if not number and not tuple then
    vim.notify('[Lspsystem] no action number choice', vim.log.levels.WARN)
    return
  end
