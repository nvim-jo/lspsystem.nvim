...
local function get_uri_data(result)
  ...
  if not res.uri or not range then
    vim.notify('[Lspsystem] Did not find target uri', vim.log.levels.WARN)
    return
  end
...
function def:peek_definition(method)
  ...
  if in_process == cur_winid then
    vim.notify(
      '[Lspsystem] There is already a peek_definition request, please wait for the response.',
      vim.log.levels.WARN
    )
    return
  end
  ...
  lsp.buf_request_all(current_buf, method_name, params, function(results)
    ...
    if not results or next(results) == nil then
      vim.notify(
        '[Lspsystem] response of request method ' .. method_name .. ' is nil',
        vim.log.levels.WARN
      )
      return
    end
