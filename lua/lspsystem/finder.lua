...
local ns_id = api.nvim_create_namespace('lspsystemfinder')
local co = coroutine
...
function finder:create_finder_data(result, method)
  ...
  for i, res in ipairs(result) do
    ...
    if not uri then
      vim.notify('[Lspsystem] miss uri in server response', vim.log.levels.WARN)
      return
    end
...
function finder:render_finder()
  ...
  self.render_fn = co.create(function(need_yield)
    ...
    for i, method in pairs(methods()) do
      ...
      for fname, item in pairs(meth_data.data) do
        ...
        api.nvim_buf_add_highlight(self.bufnr, ns_id, 'SystemCollapse', line_count - 1, 0, 5)
    ...
    if api.nvim_buf_line_count(self.bufnr) == 0 then
      clean_ctx()
      vim.notify('[Lspsystem] finder nothing to show', vim.log.levels.WARN)
      return
    end
  ...
  while true do
    ...
    if not float_width then
      print('[lspsystem] no data to show')
      return
    end
...
function finder:create_finder_win(width)
  self.group = api.nvim_create_augroup('lspsystem_finder', { clear = true })
...
function finder:apply_map()
  ...
  local function expand_or_collapse(text, curline)
    ...
    if not self.lspdata[nodes[1].method].data[nodes[1].fname].expand then
      ...
      api.nvim_buf_add_highlight(self.bufnr, ns_id, 'SystemCollapse', curline - 1, 0, 5)
      vim.bo[self.bufnr].modifiable = false
    ...
    api.nvim_buf_add_highlight(self.bufnr, ns_id, 'SystemExpand', nodes[1].start - 1, 0, 5)
...
local function create_preview_window(finder_winid)
  ...
  if not opts.col then
    vim.notify(
      '[Lspsystem] finder previee get col failed try change finder.min_width',
      vim.log.levels.WARN
    )
    return
  end
