    ...
    api.nvim_create_autocmd({ 'CursorMoved', 'InsertEnter', 'BufDelete', 'WinScrolled' }, {
      ...
      callback = function(opt)
        ...
        if opt.event == 'WinScrolled' then
          vim.cmd('Lspsystem hover_doc')
        end
        api.nvim_del_autocmd(opt.id)
      end,
      desc = '[Lspsystem] Auto close hover window',
    })
...
function hover:do_request(args)
  ...
  if count == 0 and should_error(args) then
    ...
    vim.notify('[Lspsystem] all server of buffer not support hover request')
    return
  end
  count = 0
...
function hover:render_hover_doc(args)
  if not check_parser() then
    vim.notify(
      '[Lspsystem.nvim] Please install markdown and markdown_inline parser in nvim-treesitter',
      vim.log.levels.WARN
    )
    return
  end
  ...
  if self.pending_request then
    print('[Lspsystem] There is already a hover request, please wait for the response.')
    return
  end
