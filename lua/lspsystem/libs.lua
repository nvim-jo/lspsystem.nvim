...
local system_conf = require('lspsystem').config
local libs = {}
local system_augroup = require('lspsystem').system_augroup
...
function libs.check_lsp_active(silent)
  ...
  if next(active_clients) == nil then
    if not silent then
      vim.notify('[LspSystem] Current buffer does not have any lsp server')
    end
    return false
  end
  return true
...
function libs.get_lsp_root_dir()
  ...
  for _, client in pairs(clients) do
    if client.config.filetypes and client.config.root_dir then
      ...
      for name, fts in pairs(system_conf.server_filetype_map) do
...
function libs.get_config_lsp_filetypes()
  ...
  if next(system_conf.server_filetype_map) == nil then
    return filetypes
  end

  for _, fts in pairs(system_conf.server_filetype_map) do
...
function libs.jump_beacon(bufpos, width)
  if not system_conf.beacon.enable then
    return
  end
  ...
  timer:start(
    ...
    vim.schedule_wrap(function()
      ...
      local blend = vim.wo[winid].winblend + system_conf.beacon.frequency
      if blend > 100 then
        blend = 100
      end
