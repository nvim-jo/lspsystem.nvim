local ui = require('lspsystem').config.ui
local api = vim.api
...
local function merge_custom(kind)
  ...
  for k, v in pairs(ui.kind) do
    ...
    if not index then
      vim.notify('[lspsystem.nvim] could not find kind in default')
      return
    end
    ...
    elseif type(v) == 'string' then
      ...
      vim.notify('[Lspsystem.nvim] value must be string or table')
    end
  end
end
...
local function get_kind_group()
  ...
  res[#res + 1] = 'SystemWinbarFileIcon'
  res[#res + 1] = 'SystemWinbarSep'
  return res
end
