...
local config = require('lspsystem').config
local libs = require('lspsystem.libs')
local symbar = require('lspsystem.symbolwinbar')
local window = require('lspsystem.window')
local util = require('lspsystem.util')
...
local function get_kind()
  return require('lspsystem.lspkind').get_kind()
end
...
function ot:expand_collapse()
  ...
  if node.expand then
    ...
    node.expand = false
    api.nvim_buf_add_highlight(self.bufnr, 0, 'SystemCollapse', curline - 1, 0, 5)
  ...
  vim.bo[self.bufnr].modifiable = false
  node.expand = true
  api.nvim_buf_add_highlight(self.bufnr, 0, 'SystemExpand', curline - 1, 0, 5)
  api.nvim_buf_add_highlight(
    self.bufnr,
    0,
    prefix .. kind[node.data[1].kind][1],
...
function ot:auto_refresh()
  api.nvim_create_autocmd('BufEnter', {
    ...
    desc = '[Lspsystem.nvim] outline auto refresh',
  })
end
...
function ot:render_outline(buf, symbols)
  ...
  for k, v in pairs(res) do
    ...
    scope['SystemCount'] = { #indent_with_icon + #kind[k][1] + 1, -1 }
    scope['SystemCollapse'] = { 0, #indent_with_icon }
  ...
  api.nvim_create_autocmd('WinClosed', {
    ...
    desc = '[lspsystem.nvim] clean the outline data after the win closed',
  })
end
...
function ot:outline(buf, non_close)
  ...
  if #lsp.get_active_clients({ bufnr = buf }) == 0 then
    vim.notify('[Lspsystem.nvim] there is no server attatched this buffer')
    return
  end
  if self.pending_request then
    vim.notify('[lspsystem.nvim] there is already a request for outline please wait')
    return
  end

  local symbols = get_cache_symbols(buf)
  self.group = api.nvim_create_augroup('LspsystemOutline', { clear = false })
  self.render_buf = buf
  if not symbols then
