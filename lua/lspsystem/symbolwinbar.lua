...
local config = require('lspsystem').config.symbol_in_winbar
...
local function bar_prefix()
  return {
    prefix = '%#SystemWinbar',
    sep = '%#SystemWinbarSep#' .. config.separator .. '%*',
  }
end
...
local function bar_file_name(buf)
  ...
  for i, v in pairs(res) do
    if i == #res then
      if #data > 0 then
        items[#items + 1] = '%#SystemWinbarFileIcon#' .. data[1] .. '%*'

        local ok, conf = pcall(api.nvim_get_hl_by_name, 'SystemWinbarFileIcon', true)
        if not ok then
          conf = {}
        end
...
function symbar:register_events(buf)
  local augroup = api.nvim_create_augroup('LspsystemSymbol' .. tostring(buf), { clear = true })
  ...
  api.nvim_create_autocmd('CursorMoved', {
    ...
    desc = 'Lspsystem symbols render and request',
  })
...
function symbar:symbol_autocmd()
  api.nvim_create_autocmd('LspAttach', {
    group = api.nvim_create_augroup('LspsystemSymbols', { clear = false }),
    ...
    desc = 'Lspsystem get and show symbols',
  })
end
